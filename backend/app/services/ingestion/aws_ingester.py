...
async def ingest_aws_pricing(db: AsyncSession):
    ...
    try:
        ...
        # NOTE: AWS Pricing API response is ~1GB for EC2
        # In production, use streaming + filtering. For now, use fallback.
        # Live API implementation would go here with httpx streaming
        raise Exception("Live AWS API not implemented — using fallback")
    except Exception as e:
        logger.warning(f"AWS API failed: {e}. Using fallback data.")
